...
export function BorrowerCard() {
  ...
  const collateralNum = principalNum > 0 ? principalNum * 1.1 : 0;
  const collateral = collateralNum > 0 ? collateralNum.toFixed(7) : "";
        ...
        {isPastDeadline ? (
          <div className="mt-4 rounded-md border border-danger/40 bg-danger/10 p-3 text-xs text-danger">
            ...
            <p className="mt-1 text-danger/90">
              The deadline has passed. Anyone can now liquidate this loan and
              claim the collateral. Repay is no longer available.
            </p>
          </div>
        ) : (
          <button
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between rounded-md border border-border bg-bg/40 px-3 py-2 text-xs">
        <span className="text-subtle">Available to borrow right now</span>
        <span className="font-mono font-semibold text-accent">
          {pool ? `${fmtXlm(pool.availableXlm)} XLM` : "…"}
        </span>
      </div>
      <div className="grid grid-cols-2 gap-3">
        ...
        <div>
          ...
          <input
            ...
            readOnly
            tabIndex={-1}
            aria-label="Collateral, auto-calculated at 110% of principal"
            className={`${inputCls} mt-1 cursor-not-allowed bg-elevated font-mono text-subtle`}
          />
        </div>
      </div>
      <div>
      ...
      <button
        type="submit"
        disabled={borrow.isPending || durationSeconds <= 0 || principalNum <= 0}
      ...
      >
        {borrow.isPending
          ? "Opening..."
          : principalNum > 0
            ? `Open Loan: borrow ${principalNum.toFixed(4).replace(/\.?0+$/, "")} XLM, lock ${collateral} XLM`
            : "Open Loan"}
      </button>
      {err && (
